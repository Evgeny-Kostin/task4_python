...
def search_func(msg: str):
    word = view.input_info(msg) #text.input_search_word
...
def start():
    while True:
        ...
        match choice:
            ...
            case '6':
                search_func(text.input_edt_word)
                u_id = view.input_info(text.input_edt_id)
                new_contact = view.input_new_contact(text.input_edit_contact)
                name = model.edit(int(u_id), new_contact)   #т.к. в стр 31 он строковый # модел вернул name
